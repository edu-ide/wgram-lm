...
- [2026 Recursive, Memory, Context, And Search Papers](sources/2026-recursive-memory-context-papers.md): MeMo, Equilibrium Reasoners, VPO, LT2, and PEEK mapped to QTRM/MemoryOS test gates.
- [Attractor Models](sources/attractor-models.md): 2026 fixed-point attractor model reference for convergence-chosen adaptive depth and QTRM/PV-GRAM attractor training.
...
- [One-Body Architecture SSOT](architecture/one-body-architecture-ssot.md): authoritative post-Stage99 architecture diagram and hard reject rules for keeping from-scratch/general-LM work on the HRM-Text-style read-think-speak causal path.
- [Internal Multi-Trajectory Answer Attractor SSOT](architecture/internal-multitrajectory-answer-attractor-ssot.md): active paper-candidate contract for one-body GRAM/PTRM-style stochastic trajectories, answer-attractor selection, same LM-head speaking, and ablation requirements.
...
- [Qwen3.5 Preinit Core Interpolation Stabilization](decisions/qwen35-preinit-interpolation-stabilization.md): records the 128-case accepted but 256-case near-miss Qwen3.5-pretrained mandatory-core interpolation and the decision to move from alpha hunting to fixed 256-case family-balanced selection.
- [QTRM/PV-GRAM Project Milestones](decisions/qtrm-project-milestones.md): current end-to-end milestone ladder from one-body text learning to multilingual, multimodal, agentic, Codex-like, and Qwen3.6-27B comparison gates.
- [Past-Success Doubt Loop: Stage56/58 vs One-Body Language](decisions/past-success-doubt-loop-stage56-stage58.md): prevents overclaiming old PTRM selected/oracle arithmetic success as general LM ability and defines the required past-success comparison row before new long runs. Current generated report: [markdown](decisions/past-success-doubt-loop-stage56-stage58.report.md), [json](decisions/past-success-doubt-loop-stage56-stage58.report.json). Reproducibility manifest: [markdown](decisions/past-success-doubt-loop-stage56-stage58.repro_manifest.md), [json](decisions/past-success-doubt-loop-stage56-stage58.repro_manifest.json).
- [2026 Local Depth / DGX Pretrain Generalization Split](decisions/2026-local-depth-dgx-pretrain-generalization-split.md): hard-locks local 4090 to fast depth-scaling architecture gates and DGX to OPUS/Generalization-Dynamics pretraining efficiency gates.
- [2026-05-25 Stage101 Solution-Aligned Answer Attractor](decisions/2026-05-25-stage101-solution-aligned-answer-attractor.md): locks the next recurrent one-body target to answer-facing convergence, where deeper thinking must move the same LM head toward intelligence answers and away from parrot shortcuts.
- [2026-05-25 Official GDN2 Runtime Contract](decisions/2026-05-25-official-gdn2-runtime-contract.md): fail-fast runtime policy for official GDN2, explicit ptxas, and legacy fallback checkpoint rejection before Stage95-style continuation.
- [2026-05-25 Stage101A Solution-Attractor Smoke](decisions/2026-05-25-stage101a-solution-attractor-smoke.md): records the first local answer-facing contrastive smoke where GD-lite depth4 improves to 0.6667 accuracy but remains rejected due successive/truthy failures and tiny heldout loss regression.
- [2026 Paper-Driven Test Plan](decisions/2026-paper-driven-test-plan.md): paper-backed test matrix for EqR depth/breadth, PEEK/MeMo context maps, VPO diversity, and LT2 mixer loops.
- [2026 Paper-Driven Archived Recurrent Test Run](decisions/2026-paper-driven-archived-recurrent-test-run.md): current local run against archived recurrent artifacts; rejects depth scaling, marks EqR residual telemetry missing, and records MemoryOS baseline tests.
- [Qwen3.5 HRM-Text Attention Pooling Diagnostics](decisions/qwen35-hrmtext-attention-pooling-diagnostics.md): current Qwen0.8B TRM-LLM diagnostic ledger; mandates TensorBoard plus Aim for all nontrivial Qwen/QTRM/HRM-Text training, eval, seed, and ablation runs.
- [Qwen3.5 HRM-Text Attention Pooling Diagnostics](decisions/qwen35-hrmtext-attention-pooling-diagnostics.md#2026-05-21-stage35-hard-lock-qwen-preinit-hrm-text-like-gram-main-thought-path): hard-locks the next generalization route to Qwen3.5 pretrained reader -> HRM-Text-like semantic recurrent thought state -> True-GRAM state transition -> Qwen-compatible LM/token logits from that state only; toy digit executors and side-path readout/schedule/sampler tweaks are diagnostic-only.
- [Stage60 PG-GRAM Procedure-Generating GRAM](decisions/stage60-pg-gram-procedure-generating-gram.md): next big-jump contract after Stage59 v44; replaces the remaining BENAPE hard numeric/list primitive with a learned procedure generator/executor that owns the normal answer path.
- [Stage61 One-Body vs Qwen-Transplant Split](decisions/stage61-onebody-vs-qwen-transplant-split.md): separates the DGX from-scratch one-body question from the local Qwen transplant nerve/ledger diagnostic.
...
- [State-Transition-First Redesign](decisions/state-transition-first-redesign.md): accepted root redesign making state prediction the PRIMARY supervised target with operation-conditioned transitions, replacing the weak residual adapter approach.
...
- [Qwen3.5 HRM-Text Attention Pooling Diagnostics](decisions/qwen35-hrmtext-attention-pooling-diagnostics.md): Stage59 generality gate rejects frozen-mouth/free numeric generation, promotes shuffled-choice verifier selection at 0.6484, and identifies candidate exposure as the next bottleneck because Qwen-generated oracle coverage is only 0.31-0.38.
- [Stage59 Final-Only Typed Register Answer Path](decisions/stage59-final-only-typed-register-answer-path.md): locks Stage59 to the final learned typed-register answer path, demotes heuristic typed pools/noisy candidates/standalone proposers to diagnostic scaffolds, and requires ablatable final-answer gains.
