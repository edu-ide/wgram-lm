...
| Mythos-style answer-loop update | Rejected probe | `scripts/256_run_qtrm_ouro_answer_loop_joint_decoder_s040.sh` | Stable input-injected answer recurrence plus loop-index, LoRA, ACT, and joint decoder training still scores generation 0/8; full forced-choice is 2/4 while decoder-off is 4/4. A 2026-05-30 local audit confirms `fc8c9133` used Mythos LoRA rank 8 and that the rank-8 checkpoint gives a narrow causal forced-choice depth signal (depth4/depth8 2/2, donor/core-off/depth1 0/2) only without donor-logit scale-1.0 blending. Keep OpenMythos/Parcae ideas as TRM-core stability references, not a second answer-side reasoning core. |
