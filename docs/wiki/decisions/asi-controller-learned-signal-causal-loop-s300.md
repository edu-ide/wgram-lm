...
from the current latent core while the controller/action mapping was trained
together.
...
The full learned-core replacement collapses to one action on held-out rows:
`VERIFY_EVIDENCE`. This fails to recover the oracle signal scaffold and does
not show a learned world-model/verifier path. World-model-off masks controller
signal dimension 0 and verifier-off masks dimension 1, either on an external
oracle signal or on the learned core-derived signal. The ASI gate correctly
rejects because QTRM does not beat scripted/donor harness baselines and because
the latent core, world-model, and verifier paths are not causally necessary.
