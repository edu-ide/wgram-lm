...
CONFIG="${CONFIG:-configs/qwen35_2b_4090_pure_recursive_transition_joint_dynamic_halt_v3_ouro_causal_talker_only_s080.yaml}"
EVAL_CONFIG="${EVAL_CONFIG:-configs/qwen35_2b_4090_pure_recursive_transition_joint_dynamic_halt_v3_ouro_causal_talker_only_eval_gate.yaml}"
...
OUT_DIR="${OUT_DIR:-local_eval/qwen35_2b_pure_recursive_transition_joint_dynamic_halt_v3_ouro_causal_talker_only_s080_from_halt_s080}"
STEPS="${STEPS:-80}"
LR="${LR:-1.0e-4}"
...
echo "=== QTRM Ouro causal Talker-only S080 ==="
echo "config: ${CONFIG}"
...
"${PYTHON_BIN}" scripts/192_eval_raw_intelligence.py \
  ...
  --mode qtrm_core_steps_8_answer_talker_off_no_evidence \
  --out "${OUT_DIR}/generation_smoke8_eval_gate.jsonl"
...
"${PYTHON_BIN}" scripts/192_eval_raw_intelligence.py \
  ...
  --mode qtrm_core_steps_8_answer_halt_gate_off_no_evidence \
  --mode qtrm_core_steps_8_answer_talker_off_no_evidence \
  --out "${OUT_DIR}/causal_forced_choice_smoke4_eval_gate.jsonl"
