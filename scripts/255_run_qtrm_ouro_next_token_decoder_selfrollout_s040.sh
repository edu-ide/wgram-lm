...
CONFIG="${CONFIG:-configs/qwen35_2b_4090_pure_recursive_transition_joint_dynamic_halt_v3_ouro_next_token_decoder_selfrollout_s040.yaml}"
...
INIT_CHECKPOINT="${INIT_CHECKPOINT:-/mnt/nvme1n1p2/qtrm-runs/qwen35_2b_pure_recursive_transition_joint_dynamic_halt_v3_ouro_next_token_decoder_s080_from_halt_s080/last.pt}"
OUT_DIR="${OUT_DIR:-/mnt/nvme1n1p2/qtrm-runs/qwen35_2b_pure_recursive_transition_joint_dynamic_halt_v3_ouro_next_token_decoder_selfrollout_s040_from_s080}"
STEPS="${STEPS:-40}"
LR="${LR:-1.0e-4}"
SELF_ROLLOUT_WEIGHT="${SELF_ROLLOUT_WEIGHT:-0.50}"

mkdir -p "${OUT_DIR}"

echo "=== QTRM Ouro in-loop next-token decoder self-rollout S040 ==="
echo "config: ${CONFIG}"
