...
CONFIG="${CONFIG:-configs/qwen35_2b_4090_pure_recursive_transition_joint_dynamic_halt_v3_core_state_only_kiss_prev_token_readout_s040.yaml}"
...
OUT_BASE="${OUT_BASE:-/mnt/nvme0n1p2/qtrm-runs/research_gate_runner/prev_token_readout_smoke}"
STEPS="${STEPS:-5}"
...
RUN_GATE="${RUN_GATE:-1}"
TRAINABLE_PARAM_POLICY="${TRAINABLE_PARAM_POLICY:-}"
TRAIN_OUT="${TRAIN_OUT:-$OUT_BASE/train_s${STEPS}}"
GATE_OUT="${GATE_OUT:-$OUT_BASE/gate_1case_s${STEPS}}"

TRAINABLE_ARGS=()
if [[ -n "$TRAINABLE_PARAM_POLICY" ]]; then
  TRAINABLE_ARGS+=(--trainable-param-policy "$TRAINABLE_PARAM_POLICY")
fi
