...
# Generic recurrent-trajectory gate. Unlike checksum-specific residue
# supervision, this pressures every recurrent step to improve the normal
# digit-choice LM margin over the core-off Qwen path.
...
OUT_DIR="${OUT_DIR:-local_eval/qwen35_preinit_recurrent_trajadv_multiseed_s${STEPS:-80}_$(date +%Y%m%d_%H%M%S)}"
...
LANGUAGE_HEALING_WEIGHT="${LANGUAGE_HEALING_WEIGHT:-0.10}" \
...
CHECKSUM_TRAJECTORY_WEIGHT="${CHECKSUM_TRAJECTORY_WEIGHT:-0.6}" \
...
TRAJECTORY_LOSS_BASE_ERROR_ONLY="${TRAJECTORY_LOSS_BASE_ERROR_ONLY:-0}" \
STEPS="${STEPS:-80}" \
