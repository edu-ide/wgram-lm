...
CONFIG="${CONFIG:-configs/qwen35_2b_4090_workspace_evidence_path_s050.yaml}"
DATA_JSONL="${DATA_JSONL:-data/filtered/memory_reasoning_synth_traces.jsonl}"
OUT_DIR="${OUT_DIR:-runs/qwen35_2b_4090_workspace_evidence_path_s050}"
...
echo "============================================================"
echo "Workspace evidence-path training"
echo "config=${CONFIG}"
...
CONFIG="$CONFIG" CHECKPOINT="$CHECKPOINT" \
  OUT="${OUT:-runs/eval/memory_reasoning_heldout_expanded_workspace_evidence_path_32tok_trained_s050.jsonl}" \
  AUDIT_OUT="${AUDIT_OUT:-runs/eval/memory_reasoning_heldout_expanded_workspace_evidence_path_32tok_trained_s050_audit.jsonl}" \
  PROOF_MD="${PROOF_MD:-docs/wiki/decisions/workspace-evidence-path-trained-ablation.md}" \
  PROOF_JSON="${PROOF_JSON:-docs/wiki/decisions/workspace-evidence-path-trained-ablation-summary.json}" \
