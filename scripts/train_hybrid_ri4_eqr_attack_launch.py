...
================================================================================
"""
[EqR H/L attack run - header minimized for launch]
"""
# Robust bootstrap
import os, sys
...
def parse_continuation_args() -> ContinuationConfig:
    ...
    p.add_argument("--enable_ri1_variable_depth", action="store_true", help="Enable M1 variable recurrence depth sampling during training (Huginn-style). This is the main architectural move to instill the 'deeper is better' inductive bias for RI-1. Strongly recommended for serious depth scaling work. Composes with Attractor depth-wise monotonic pressure.")
    ...
    # === New EqR aggressive substrate controls (added for the H/L + NI attack) ===
    p.add_argument("--attractor_h_cycles", type=int, default=2, help="EqR H-cycles (slow high-level goal maintenance).")
    p.add_argument("--attractor_l_cycles", type=int, default=6, help="EqR L-cycles (fast low-level noisy refinement per H).")
    p.add_argument("--attractor_ri_scale", type=float, default=0.08, help="RI (randomized init) scale for solver y0 at start of solve (EqR basin shaping).")
    p.add_argument("--attractor_l_noise_base", type=float, default=0.022, help="Base noise injection strength inside L-cycles (the critical NI lever from EqR).")
    p.add_argument("--attractor_lambda_mix", type=float, default=0.91, help="Lambda for residual mixing inside solver steps (lower = more aggressive update).")
    p.add_argument("--attractor_noise_scale", type=float, default=0.012, help="General noise scale fallback.")
    ...
    p.add_argument("--l3_reasoning_path", type=str, default=None, help="Path to L3-generated reasoning traces (from generate_l3_reasoning_traces.py). When provided + depth_consistency_weight > 0, the trainer samples real short-vs-deep pairs from the data (UltraData L3 style) instead of crude internal rollouts. Strongly recommended for proper RI-1 depth consistency training.")
    ...
    if getattr(cfg, 'internal_fast_recurrent', False) and not getattr(args, 'ri1_depth_ablation_fixed', False):
        ...
        # Aggressive boost for Equilibrium Internalization (Solve-the-Loop + EqR priority)
        if getattr(cfg, 'use_explicit_attractor_solver', False):
            if getattr(cfg, 'attractor_internalization_weight', 0.12) < 0.30:
                cfg.attractor_internalization_weight = max(cfg.attractor_internalization_weight, 0.30)
                print("  [AGGRESSIVE EqR SUBSTRATE] Bumped attractor_internalization_weight → 0.30 (H/L + NI attack)")
        else:
            if getattr(cfg, 'attractor_internalization_weight', 0.12) < 0.25:
                cfg.attractor_internalization_weight = max(cfg.attractor_internalization_weight, 0.25)
                print("  [Aggressive Internalization] Bumped attractor_internalization_weight to 0.25+ for stronger equilibrium internalization drive")
    if ri1_variable_active and not getattr(args, 'ri1_depth_ablation_fixed', False):
        ...
        cfg.ri1_depth_curriculum_ramp = True  # Gradually increase mean depth during training

        print("\n" + "="*70)
        print("[RI-1 ARCHITECTURAL IMPROVEMENT] Variable Depth Training ACTIVE")
        print("  Goal: Instill 'deeper is better' inductive bias so that increasing depth reliably improves performance.")
        print("  Sampling: %s | initial_mean=%s max=%s" % (cfg.ri1_depth_sampling_mode, cfg.ri1_depth_mean, cfg.core_elastic_depth_max_steps))
        print("  Curriculum ramp enabled: mean depth will gradually increase over training steps.")
        print("  Depth-wise monotonic pressure via Attractor on memory_buffer states is active.")
        print("  This is the core training change for RI-1 depth scaling (beyond just inference-time depth).")
        print("="*70 + "\n")
    else:
        cfg.ri1_variable_depth_active = False

    # === L3 Reasoning Data (UltraData L3 for RI-1 depth consistency) ===
    # Generated by scripts/generate_l3_reasoning_traces.py
    # Attached to cfg for reliable access from the training loop (no scoping issues).
    cfg.l3_reasoning_traces = []
    l3_path = getattr(args, 'l3_reasoning_path', None)
    if l3_path:
        try:
            with open(l3_path, 'r', encoding='utf-8') as f:
                for line in f:
                    if line.strip():
                        cfg.l3_reasoning_traces.append(json.loads(line))
            print(f"\n[L3 UltraData] Loaded {len(cfg.l3_reasoning_traces)} real L3 reasoning traces")
            print("  These will be actively used every consistency step with real questions + why_deeper_better.")
        except Exception as e:
            print(f"[L3 UltraData] Failed to load {l3_path}: {e}")
            cfg.l3_reasoning_traces = []
...
def _sample_ri1_effective_depth(cfg: ContinuationConfig, step: int, total_steps: int = None) -> int:
    """
    RI-1 M1 v2 (aggressive Huginn/LoopFormer-style for strong "deeper is better" bias):
    - Steep curriculum ramp: mean depth rises fast after mid-training.
    - Late training: very strong bias toward high/max depth (the model must learn that extra steps help).
    - Heavy tail + forced max samples increase with progress.
    - This is the core mechanism to instill monotonic depth scaling inductive bias during training.
    """
    ...
    # Aggressive curriculum ramp (steeper than v1)
    if getattr(cfg, 'ri1_depth_curriculum_ramp', False):
        ramped_mean = d_mean + int(progress * (d_max - d_mean) * 0.85)  # was 0.6
        d_mean = min(ramped_mean, d_max - 1)

    if mode == 'lognormal_poisson':
        effective_mean = d_mean + int((d_max - d_mean) * progress * 0.8)
        r = random.randint(1, d_max)
        # Much stronger heavy tail in late training
        tail_p = 0.25 + 0.55 * progress
        if random.random() < tail_p:
            r = min(d_max, max(r, int(effective_mean * (1.4 + random.random() * 1.6))))
        return max(1, r)
    else:
        # Much more aggressive late bias
        if progress < 0.25:
            low = max(1, d_mean - 2)
            high = d_mean + 2
        elif progress < 0.55:
            ...
            # Final 20%: heavily favor deep
            low = max(d_mean, d_max - 3)
            high = d_max

        r = random.randint(low, high)
        # Force max depth with increasing probability (core of "deeper is better" pressure)
        force_max_p = 0.15 + 0.65 * max(0.0, (progress - 0.5))
        if random.random() < force_max_p:
            r = d_max
        return max(1, min(r, d_max))


def main():
    # Ensure L3 traces variable is always defined in function scope (for the depth consistency block)
    L3_REASONING_TRACES: List[Dict[str, Any]] = []
    ...
    if getattr(cfg, 'use_explicit_attractor_solver', False):
        if not _ATTRACTOR_AVAILABLE:
            ...
            try:
                ...
                # === AGGRESSIVE EqR SUBSTRATE (H/L + strong NI/RI + latent carry) ===
                # Now using the June 2026 full attack version of AttractorSolverModule.
                # These defaults are deliberately strong to test whether proper hierarchical
                # noisy fixed-point iteration finally delivers monotonic RI-1 depth scaling.
                explicit_solver = AttractorSolverModule(
                    ...
                    max_solver_steps=getattr(cfg, 'attractor_solver_max_steps', 16),  # give the hierarchical solver room
    ...
    for step in range(start_step, start_step + total_to_run):
        ...
        if getattr(cfg, 'use_explicit_attractor_solver', False) and sot_trainer is not None and explicit_solver is not None:
            try:
                ...
                # === Strong Equilibrium Internalization (Solve-the-Loop style - aggressive version) ===
                # Core idea from the paper: drive the backbone proposal y0 to be close to the final attractor equilibrium.
                # This is the highest-leverage signal for making "deeper is better" emerge in the backbone itself.
                int_mse = torch.nn.functional.mse_loss(proposal, equilibrium.detach())

                # Stronger multi-term internalization (paper's spirit + practical stabilization)
                int_cosine = 1.0 - torch.nn.functional.cosine_similarity(
                    proposal.flatten(1), equilibrium.detach().flatten(1), dim=1
                ).mean()

                # Optional: small term encouraging the proposal to already be a decent predictor
                # (helps internalization even when solver budget is low at inference)
                int_pred = torch.nn.functional.mse_loss(
                    proposal.detach(), equilibrium.detach()
                ) if getattr(cfg, 'attractor_internalization_pred_aux', 0.0) > 0 else torch.tensor(0.0, device=h.device)

                int_total = int_mse + 0.2 * int_cosine + 0.1 * int_pred
                ...
                int_contrib_t = int_total * int_w

                # Store rich metrics for logging
                setattr(cfg, '_int_mse', float(int_mse))
                setattr(cfg, '_int_cosine', float(int_cosine))
                setattr(cfg, '_int_total_raw', float(int_total))
                ...
                if (step + 1) % 2 == 0:
                    ...
                    int_mse_val = getattr(cfg, '_int_mse', int_val)
                    int_cos_val = getattr(cfg, '_int_cosine', 0.0)
                    print(f"  [Section 7 Attractor] step {step+1:02d} | sot={sot_val:.5f} int_total={float(int_total):.5f} "
                          f"(mse={int_mse_val:.5f}, cos={int_cos_val:.4f}) Δ={delta:+.5f} densing_sig≈{1.0/(int_mse_val+1e-6):.2f}")
            except Exception as e:
        ...
        if getattr(cfg, 'ri1_variable_depth_active', False) and depth_consistency_w > 0.0 and (step + 1) % max(1, getattr(cfg, 'heldout_answer_pressure_interval', 3)) == 0:
            ...
            if eff_depth >= 4:  # only apply when we actually sampled meaningfully longer depth
                # === L3 UltraData Integration (real short-vs-deep pairs from our generated traces) ===
                l3_pair = None
                l3_question = None
                l3_why = None
                l3_traces = getattr(cfg, 'l3_reasoning_traces', [])
                if l3_traces:
                    try:
                        l3_pair = random.choice(l3_traces)
                        l3_short_deep = l3_pair.get("short_vs_deep_for_loss", {})
                        l3_question = l3_pair.get("original_question", "")
                        l3_why = l3_short_deep.get("why_deeper_better", "")
                        # Always log when we have real L3 data (much stronger signal than synthetic)
                        print(f"  [L3 UltraData] Using real pair | why: {l3_why[:100]}...")
                    except Exception:
                        l3_pair = None

                try:
                    with torch.no_grad():
                        # Use real L3 question as seed when available (much better than pure random synthetic input)
                        seed_text = l3_question if l3_question else ""
                        h_short = make_input(step, 1) * 0.0
                        # crude short rollout (depth ~2)
                        for _ in range(2):
                            for layer in model:
                                if isinstance(layer, OneBodyParallelHybridBlock):
                    ...
                    # Long rollout using the same eff_depth
                    ...
                    margin = 0.10 if l3_pair else 0.08   # slightly stronger margin when using real L3 data
                    ...
                    log_msg = f"  [RI-1 Depth Consistency] eff_depth={eff_depth} consistency_loss={float(consistency_loss):.4f}"
                    if l3_pair:
                        log_msg += " (L3 real pair)"
                    if (step + 1) % max(1, cfg.total_steps // 4) == 0 or l3_pair:
                        print(log_msg)
