...
CONFIG="${CONFIG:-configs/qwen35_2b_4090_workspace_evidence_preference_s050.yaml}"
DATA_JSONL="${DATA_JSONL:-data/filtered/memory_self_improvement_preferences_analysis.jsonl}"
OUT_DIR="${OUT_DIR:-runs/qwen35_2b_4090_workspace_evidence_preference_s050}"
...
PREFERENCE_EVAL_OUT="${PREFERENCE_EVAL_OUT:-runs/eval/workspace_evidence_preference_pair_eval_s050.jsonl}"

echo "============================================================"
echo "Workspace evidence-path preference training"
echo "config=${CONFIG}"
...
CONFIG="$CONFIG" CHECKPOINT="$CHECKPOINT" \
  OUT="${OUT:-runs/eval/memory_reasoning_heldout_expanded_workspace_evidence_preference_32tok_trained_s050.jsonl}" \
  AUDIT_OUT="${AUDIT_OUT:-runs/eval/memory_reasoning_heldout_expanded_workspace_evidence_preference_32tok_trained_s050_audit.jsonl}" \
  PROOF_MD="${PROOF_MD:-docs/wiki/decisions/workspace-evidence-preference-trained-ablation.md}" \
  PROOF_JSON="${PROOF_JSON:-docs/wiki/decisions/workspace-evidence-preference-trained-ablation-summary.json}" \
