    ...
    *,
    ...
    for depth in range(1, int(num_steps) + 1):
        ...
        if value is None:
            ...
            continue
        token_ids = answer_token_ids(tokenizer, str(value))[: int(max_target_tokens)]
...
def predicted_token_sequences_from_logits(logits: Any) -> list[list[int]]:
    if (
        logits.ndim != 4
...
def select_sequence_logits(outputs: dict[str, Any], *, logits_key: str):
    if logits_key not in outputs:
        raise KeyError(f"missing sequence logits key: {logits_key}")
    return outputs[logits_key]


def score_sequence_predictions(
    *,
...
def _summarize_records(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for record in records:
        ...
        bucket["correct_tokens"] += int(record["correct_tokens"])
        bucket["total_tokens"] += int(record["total_tokens"])
        bucket["exact_steps"] += int(record["exact_steps"])
        bucket["total_steps"] += int(record["total_steps"])
    for bucket in by_family.values():
        bucket["token_accuracy"] = (
            float(bucket["correct_tokens"]) / float(bucket["total_tokens"])
            if int(bucket["total_tokens"])
            else 0.0
        )
        bucket["step_exact_accuracy"] = (
            float(bucket["exact_steps"]) / float(bucket["total_steps"])
...
def evaluate_rows(
    *,
    ...
    logits_key: str = "core_depth_text_logits",
    ...
    try:
        with torch.no_grad():
            for row in rows:
                ...
                logits = select_sequence_logits(outputs, logits_key=logits_key)
                token_count = min(int(max_target_tokens), int(logits.shape[2]))
                predicted = [
                    [int(token_id) for token_id in sequence[:token_count]]
                    for sequence in predicted_token_sequences_from_logits(
                        logits[:, :, :token_count, :]
                    )
                ]
                targets = sequence_targets_from_row(
                    tokenizer,
                    row,
                    num_steps=len(predicted),
                    max_target_tokens=token_count,
                )
                score = score_sequence_predictions(
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate QTRM core-depth full state text sequence predictions."
    )
    ...
    parser.add_argument(
        "--logits-key",
        default="core_depth_text_logits",
        choices=["core_depth_text_logits", "transition_state_sequence_logits"],
    )
