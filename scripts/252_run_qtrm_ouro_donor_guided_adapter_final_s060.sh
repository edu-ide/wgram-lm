...
OUT_DIR="${OUT_DIR:-local_eval/qwen35_2b_pure_recursive_transition_joint_dynamic_halt_v3_ouro_donor_guided_adapter_final_s060_from_halt_s080}"
STEPS="${STEPS:-60}"
LR="${LR:-3.0e-4}"
FINAL_LOGIT_CE_WEIGHT="${FINAL_LOGIT_CE_WEIGHT:-1.0}"
TEACHER_FINAL_KL_WEIGHT="${TEACHER_FINAL_KL_WEIGHT:-0.03}"

mkdir -p "${OUT_DIR}"

echo "=== QTRM Ouro donor-guided LM-adapter final-answer S060 ==="
echo "config: ${CONFIG}"
...
"${PYTHON_BIN}" scripts/196_train_pure_recursive_depth_supervised.py \
  ...
  --final-greedy-token-margin-weight 0.35 \
  --greedy-token-margin 0.15 \
  ...
  --causal-prefix-later-token-weight 0.75
