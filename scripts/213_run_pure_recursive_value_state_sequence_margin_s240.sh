...
TRAIN_CASES="${TRAIN_CASES:-data/filtered/pure_recursive_value_state_sequence_margin_train_cases.jsonl}" \
DATA="${DATA:-data/filtered/pure_recursive_value_state_sequence_margin_preferences.jsonl}" \
OUT_DIR="${OUT_DIR:-/mnt/nvme1n1p2/qtrm-local-checkpoints/pure_recursive_value_state_sequence_margin_s240}" \
RUN_NAME="${RUN_NAME:-pure_recursive_value_state_sequence_margin_s240}" \
HELDOUT_CASES="${HELDOUT_CASES:-data/eval/pure_recursive_value_state_sequence_margin_heldout200_cases.jsonl}" \
EVAL_OUT="${EVAL_OUT:-/mnt/nvme1n1p2/qtrm-eval/pure_recursive_value_state_sequence_margin_s240_heldout200_mean_depth_gate_8.jsonl}" \
...
STAGED_INTERNAL_FIRST_TOKEN_CE_WEIGHT="${STAGED_INTERNAL_FIRST_TOKEN_CE_WEIGHT:-0.30}" \
...
TRANSITION_STATE_CE_WEIGHT="${TRANSITION_STATE_CE_WEIGHT:-0.50}" \
