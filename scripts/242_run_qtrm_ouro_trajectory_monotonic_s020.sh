...
CONFIG="${CONFIG:-configs/qwen35_2b_4090_pure_recursive_transition_joint_dynamic_halt_v3_ouro_trajectory_monotonic_s020.yaml}"
...
OUT_DIR="${OUT_DIR:-local_eval/qwen35_2b_pure_recursive_transition_joint_dynamic_halt_v3_ouro_trajectory_monotonic_s020_from_tail_s020}"
STEPS="${STEPS:-20}"
LR="${LR:-1.0e-5}"
DEPTH_TRAJECTORY_MONOTONIC_WEIGHT="${DEPTH_TRAJECTORY_MONOTONIC_WEIGHT:-0.25}"
DEPTH_TRAJECTORY_MONOTONIC_MARGIN="${DEPTH_TRAJECTORY_MONOTONIC_MARGIN:-0.02}"
TAIL_NEGATIVE_MARGIN_WEIGHT="${TAIL_NEGATIVE_MARGIN_WEIGHT:-0.25}"
TAIL_NEGATIVE_MARGIN="${TAIL_NEGATIVE_MARGIN:-0.07}"

echo "=== QTRM Ouro trajectory-monotonic S020 ==="
echo "config: ${CONFIG}"
...
python scripts/196_train_pure_recursive_depth_supervised.py \
  ...
  --depth-trajectory-monotonic-weight "${DEPTH_TRAJECTORY_MONOTONIC_WEIGHT}" \
  --depth-trajectory-monotonic-margin "${DEPTH_TRAJECTORY_MONOTONIC_MARGIN}" \
  ...
  --tail-negative-margin "${TAIL_NEGATIVE_MARGIN}" \
