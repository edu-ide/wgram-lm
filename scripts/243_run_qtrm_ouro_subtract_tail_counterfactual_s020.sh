...
CONFIG="${CONFIG:-configs/qwen35_2b_4090_pure_recursive_transition_joint_dynamic_halt_v3_ouro_joint_answer_bridge_s020.yaml}"
...
OUT_DIR="${OUT_DIR:-local_eval/qwen35_2b_pure_recursive_transition_joint_dynamic_halt_v3_ouro_subtract_tail_counterfactual_s020_from_tail_s020}"
STEPS="${STEPS:-20}"
LR="${LR:-1.0e-5}"
SUBTRACT_TAIL_COUNTERFACTUAL_MARGIN_WEIGHT="${SUBTRACT_TAIL_COUNTERFACTUAL_MARGIN_WEIGHT:-0.50}"
SUBTRACT_TAIL_COUNTERFACTUAL_MARGIN="${SUBTRACT_TAIL_COUNTERFACTUAL_MARGIN:-0.05}"

echo "=== QTRM Ouro subtract-tail counterfactual S020 ==="
echo "config: ${CONFIG}"
...
python scripts/196_train_pure_recursive_depth_supervised.py \
  ...
  --subtract-tail-counterfactual-margin "${SUBTRACT_TAIL_COUNTERFACTUAL_MARGIN}" \
