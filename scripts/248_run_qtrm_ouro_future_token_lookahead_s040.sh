...
CONFIG="${CONFIG:-configs/qwen35_2b_4090_pure_recursive_transition_joint_dynamic_halt_v3_ouro_future_token_lookahead_s040.yaml}"
...
OUT_DIR="${OUT_DIR:-local_eval/qwen35_2b_pure_recursive_transition_joint_dynamic_halt_v3_ouro_future_token_lookahead_s040_from_halt_s080}"
STEPS="${STEPS:-40}"
LR="${LR:-5.0e-5}"
FUTURE_TOKEN_CE_WEIGHT="${FUTURE_TOKEN_CE_WEIGHT:-1.0}"
DEPTH_FINAL_CE_WEIGHT="${DEPTH_FINAL_CE_WEIGHT:-0.20}"
HALT_CE_WEIGHT="${HALT_CE_WEIGHT:-0.25}"
TEACHER_DEPTH_KL_WEIGHT="${TEACHER_DEPTH_KL_WEIGHT:-0.05}"

mkdir -p "${OUT_DIR}"

echo "=== QTRM Ouro future-token lookahead S040 ==="
echo "config: ${CONFIG}"
...
"${PYTHON_BIN}" scripts/196_train_pure_recursive_depth_supervised.py \
  ...
  --answer-state-loop-future-token-ce-weight "${FUTURE_TOKEN_CE_WEIGHT}" \
  --answer-state-loop-future-token-max-target-tokens 8 \
...
"${PYTHON_BIN}" scripts/192_eval_raw_intelligence.py \
  ...
  --out "${OUT_DIR}/lm_causal_forced_choice_smoke8.jsonl"
