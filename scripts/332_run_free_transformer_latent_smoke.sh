...
CONFIG="${CONFIG:-configs/qwen35_2b_4090_pure_recursive_transition_joint_dynamic_halt_v3_core_state_only_kiss_free_transformer_latent_s040.yaml}"
...
OUT_BASE="${OUT_BASE:-/mnt/nvme0n1p2/qtrm-runs/research_gate_runner/free_transformer_latent_smoke}"
TRAIN_OUT="${TRAIN_OUT:-$OUT_BASE/train_s${STEPS:-5}}"
GATE_OUT="${GATE_OUT:-$OUT_BASE/gate_1case_s${STEPS:-5}}"
STEPS="${STEPS:-5}"
...
.venv/bin/python scripts/196_train_pure_recursive_depth_supervised.py \
  ...
  --answer-free-transformer-latent-kl-weight 0.05 \
  --answer-free-transformer-latent-free-bits 0.05 \
  --answer-free-transformer-latent-final-contrast-weight 1.0 \
  --answer-free-transformer-latent-final-contrast-margin 0.10 \
