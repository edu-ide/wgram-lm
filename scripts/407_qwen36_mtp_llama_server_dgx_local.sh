...
OpenAI-compatible URL from other machines:
  http://192.168.219.113:${PORT}/v1
...
start_server() {
  ...
  [ -x "${SERVER_BIN}" ] || { echo "missing llama-server: ${SERVER_BIN}" >&2; exit 1; }
...
run_test() {
  health || { echo "server is not ready; run start first" >&2; exit 1; }
...
case "${ACTION}" in
  -h|--help|help) usage ;;
  url) echo "http://192.168.219.113:${PORT}/v1" ;;
  start) start_server ;;
