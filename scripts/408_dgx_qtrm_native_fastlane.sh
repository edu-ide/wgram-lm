...
PLAN
  ...
  m7-final-token-repair)
    remote "PYTHON='${REMOTE_PYTHON}' PYTHONPATH=src OUT_ROOT='local_eval/dgx_qtrm_native_m7_final_token_repair_${OUT_TAG}' DEVICE=cuda STEPS='${STEPS:-1200}' BATCH_SIZE='${BATCH_SIZE:-32}' LR='${LR:-1.0e-4}' MAX_TRAIN_CASES='${MAX_TRAIN_CASES:-256}' MAX_EVAL_CASES='${MAX_EVAL_CASES:-256}' THINK_STEPS='${THINK_STEPS:-4}' bash scripts/401_run_qtrm_native_m7a_final_token_healing.sh"
    ;;
