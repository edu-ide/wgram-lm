...
RUN="${TS}_PERSIST_STAGE31A_LSCR_SDST_miniGatedDelta_seed122"
OUT="/mnt/sdc1/tripleyoung/qtrm_eval/${RUN}"
LOG="/tmp/${RUN}.log"
mkdir -p "${OUT}"

echo "Launching training run: ${RUN}"
echo "Logging to: ${LOG}"
...
env PYTHONUNBUFFERED=1 QTRM_AIM_REPO=/mnt/sdc1/tripleyoung/qtrm_aim PYTHONPATH=src \
  ...
  --aim-description "Stage31A Local LSCR plus SDST with mini_gated_delta recurrent core, seed 122" \
