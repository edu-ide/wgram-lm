...
# Stage61C-D attention-free control.
#
# Plain-language contract:
#   Same local from-scratch one-body task as the official GDN2-backed 3:1 run,
#   but remove the periodic attention re-read. This tests whether "only edit
#   working memory with GDN2" is better than "three GDN2 edits, then one
#   attention re-read".
#
# Architecture:
#   capacity-matched attention-free stack:
#     official GDN2 -> official GDN2 -> official GDN2 -> official GDN2
#   per reader/thought/speaker stage.

OUT_DIR="${OUT_DIR:-/tmp/stage61c_official_gdn2_attention_free_$(date +%Y%m%d_%H%M%S)}"
STEPS="${STEPS:-400}"
...
PYTHONUNBUFFERED=1 PYTHONPATH=src .venv/bin/python scripts/337_train_qtrm_native_mixed_text_reasoning_probe.py \
  --out-dir "${OUT_DIR}" \
  --target-level "Stage61C-D official GDN2 attention-free single-thought from scratch" \
  --steps "${STEPS}" \
  ...
  --accepted-decision "accepted_stage61c_official_gdn2_attention_free" \
  --log-every "${LOG_EVERY}" \
  ${EXTRA_ARGS:-}

echo "Stage61C-D official GDN2 attention-free run wrote: ${OUT_DIR}"
