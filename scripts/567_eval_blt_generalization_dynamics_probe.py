#!/usr/bin/env python3
"""Evaluate BLT checkpoints on a Generalization Dynamics style choice gate."""
...
def load_depth_probe_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("blt_depth_probe_for_gd_lite", path)
...
def build_choice_tensors(
    *,
    ...
    prompt_ids = encode_tokenizer_free_bytes(prompt, byte_offset=int(byte_offset))
    answer_ids = encode_tokenizer_free_bytes(answer, byte_offset=int(byte_offset))
    if not prompt_ids:
    ...
    if len(input_ids) > int(seq_len):
        raise ValueError(
            f"choice row exceeds seq_len={seq_len}: prompt+answer shifted length={len(input_ids)}"
        )
...
def choice_logprob(
    ...
    *,
    ...
    input_ids, labels, attention_mask = build_choice_tensors(
        prompt=prompt,
        answer=answer,
        seq_len=int(seq_len),
...
def infer_gate_type(rows: list[dict[str, Any]]) -> str:
    sources = " ".join(str(row.get("source", "")) for row in rows)
    if "GDsuite" in sources or "gdsuite" in sources:
        return "generalization_dynamics_official_choice_probe"
    return "generalization_dynamics_lite_choice_probe"
...
def run_eval(args: argparse.Namespace) -> dict[str, Any]:
    ...
    trainer, _prefix, ckpt_args, loaded = depth_probe.load_checkpoint_model(
        ...
        out_dir=str(Path(args.out).parent if str(args.out) else "local_eval/gd_lite_probe"),
        device=device,
    ...
    rows: list[dict[str, Any]] = []
    for row in probe_rows[: int(args.max_rows) if int(args.max_rows) > 0 else None]:
    ...
    report = {
        "gate_type": infer_gate_type(probe_rows),
        ...
        "rows": rows,
        "plain_language_read": (
            "This gate checks whether the checkpoint prefers the answer that "
            "requires generalizing over the tempting parrot/pattern answer. "
            "Use it for checkpoint selection and OPUS-style data selection; "
            "do not treat ordinary held-out loss alone as proof of generalization."
        ),
    }
    if str(args.tensorboard_dir):
        tb_step = int(args.tensorboard_step if int(args.tensorboard_step) >= 0 else 0)
        report["tensorboard_scalars_written"] = int(
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--tensorboard-prefix", default="eval/generalization_dynamics")
