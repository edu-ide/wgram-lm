...
class WGRAMV2FastlaneTests(unittest.TestCase):
    def test_fastlane_plan_is_single_core_recipe_not_comparison_sweep(self) -> None:
        ...
        self.assertTrue(plan["recipe"]["answer_memory_prompt_context"])
        self.assertEqual(plan["recipe"]["answer_memory_prompt_context_gate_init"], -1.0)
        self.assertEqual(plan["recipe"]["answer_memory_prompt_context_default_scale"], 1.0)
        self.assertEqual(plan["recipe"]["answer_memory_prompt_context_start_after"], 105)
        self.assertEqual(plan["recipe"]["answer_memory_prompt_context_warmup_steps"], 90)
        ...
        self.assertIn("--answer-memory-prompt-context", plan["train_command"])
        self.assertIn("--answer-memory-prompt-context-gate-init -1.0", plan["train_command"])
        self.assertIn("--answer-memory-prompt-context-default-scale 1.0", plan["train_command"])
        self.assertIn("--answer-memory-prompt-context-start-after 105", plan["train_command"])
        self.assertIn("--answer-memory-prompt-context-warmup-steps 90", plan["train_command"])
