...
class BLTDByteLatentPrefixLM(nn.Module):
    ...
    def _hnet_boundary_states(
        ...
        *,
        ...
        for row_idx, row_positions in enumerate(selected_positions):
            ...
            selected_embeddings[row_idx, : len(row_positions)] = byte_embeddings[row_idx, index_tensor]
        ...
        selected_hidden = self._global_hidden(selected_embeddings, think_steps=int(think_steps))
        ema_hidden = torch.zeros_like(selected_hidden)
        running = torch.zeros((batch, self.d_model), dtype=selected_hidden.dtype, device=input_ids.device)
        selected_probs = selected_probs.clamp(min=1e-4, max=1.0 - 1e-4).to(selected_hidden.dtype)
        for selected_idx in range(max_selected):
            p = selected_probs[:, selected_idx].unsqueeze(-1)
            running = p * selected_hidden[:, selected_idx] + (1.0 - p) * running
            ema_hidden[:, selected_idx] = running
        dechunked = torch.gather(
            ema_hidden,
            dim=1,
            index=dechunk_indices.unsqueeze(-1).expand(-1, -1, self.d_model),
        )
        token_hidden = byte_embeddings + boundary_probs.unsqueeze(-1).to(dechunked.dtype) * dechunked
    ...
    def _hnet_forward_losses(
        ...
        *,
        ...
        token_hidden, _, valid_mask, boundary_probs, boundary_rate, hnet_metrics = self._hnet_boundary_states(
            input_ids,
            attention_mask,
            think_steps=int(think_steps),
        )
        pre_readback_hidden = token_hidden
        token_hidden = self.apply_answer_readback(
            token_hidden,
            self.answer_embedding_weight(),
            attention_mask=attention_mask,
        )
        clean_logits = self.hnet_byte_speaker(token_hidden)
    ...
    def answer_embedding_weight(self) -> torch.Tensor:
        if self.patch_boundary_mode in {"hnet_dechunk", "hnetpp_flow_dechunk"}:
            return self.hnet_byte_speaker[-1].weight
    ...
    def speaker_logits_from_hidden(self, hidden: torch.Tensor) -> torch.Tensor:
        if self.patch_boundary_mode in {"hnet_dechunk", "hnetpp_flow_dechunk"}:
            return self.hnet_byte_speaker(hidden)
    ...
    def forward_logits_and_decoder_hidden(
        ...
        *,
        ...
        if self.patch_boundary_mode in {"hnet_dechunk", "hnetpp_flow_dechunk"}:
            ...
            return self.hnet_byte_speaker(token_hidden), token_hidden
