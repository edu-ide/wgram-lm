...
def speaker_vocab_logits(qtrm_model: Any, speaker: StateTextSpeaker, readout_state: torch.Tensor) -> torch.Tensor:
    answer_states = speaker(readout_state)
    _, vocab_logits = qtrm_model._lm_head_logits_from_state(answer_states)
    return vocab_logits
