...
def _normalize_delta_backend(delta_backend: str) -> str:
    backend = str(delta_backend).strip().lower().replace("-", "_")
    aliases = {
        "gated_delta": "torch_gated_delta",
        "torch_gated_delta2_v2": "torch_gated_delta",
        "gated_delta2_v2": "official_gated_delta2",
        "gdn2_v2": "official_gated_delta2",
        "official_gdn2": "official_gated_delta2",
        "fla_gdn": "fla_gated_delta",
    }
...
def build_qtrm_core_config_from_qwen(
    ...
    *,
    ...
    outer_steps: int = 1,
    dropout: float = 0.0,
    delta_backend: str = "torch_gated_delta",  # also accepts: fla_gated_delta, gdn2_v2
        ...
        **kwargs: Any,
        ...
        if core_config is None:
            core_config = build_qtrm_core_config_from_qwen(
                ...
                delta_backend=_normalize_delta_backend(str(kwargs.pop("delta_backend", "torch_gated_delta"))),
...
# Public W-GRAM aliases. The QTRM names remain for checkpoint/config
# compatibility with existing experiments.
QwenBackboneWGRAM = QwenBackboneQTRM
QwenBackboneWGRAMReport = QwenBackboneQTRMReport
build_wgram_core_config_from_qwen = build_qtrm_core_config_from_qwen
