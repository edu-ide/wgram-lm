...
class BLTDLocalDecoder(nn.Module):
    ...
    def _causal_mask(self, device: torch.device, *, length: int | None = None) -> torch.Tensor | None:
        if not self.causal:
            return None
        seq_len = int(length) if length is not None else int(self.patch_size)
        mask = torch.full(
            (seq_len, seq_len),
            float("-inf"),
            device=device,
    ...
    def forward_hidden(
        ...
        *,
        ...
        mask = self._causal_mask(x.device, length=int(x.shape[1]))
        h = x
