...
class OfficialGatedDeltaNet2Mixer(nn.Module):
    ...
    def __init__(
        ...
        strict: bool = False,
        fallback_dropout: float = 0.0,
        force_chunk_eval: bool = True,
        **kwargs,
    ):
        super().__init__()
        self.strict = bool(strict)
        self.force_chunk_eval = bool(force_chunk_eval)
    ...
    def forward(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        if (
            bool(self.force_chunk_eval)
            and not bool(self.training)
            and hasattr(self.impl, "mode")
            and str(getattr(self.impl, "mode")) == "chunk"
        ):
            was_training = bool(self.impl.training)
            self.impl.train(True)
            try:
                out = self.impl(x, attention_mask=attention_mask, use_cache=False)
            finally:
                self.impl.train(was_training)
        else:
            out = self.impl(x, attention_mask=attention_mask)
        if isinstance(out, tuple):
            out = out[0]
