...
class HashTokenizer:
    ...
    def _encode_content(self, text: str) -> list[int]:
        ...
        return [self._token_id(p) for p in pieces]

    def encode(self, text: str, max_len: int) -> torch.Tensor:
        ids = [self.bos_id]
        ids.extend(self._encode_content(text)[: max_len - 2])
        ids.append(self.eos_id)
        if len(ids) < max_len:
            ids.extend([self.pad_id] * (max_len - len(ids)))
        return torch.tensor(ids[:max_len], dtype=torch.long)

    def encode_supervised(self, prompt: str, answer: str, max_len: int) -> dict[str, torch.Tensor | int]:
        return _pack_supervised_ids(
            self._encode_content(prompt),
            self._encode_content(answer),
            max_len=max_len,
            bos_id=self.bos_id,
            eos_id=self.eos_id,
            pad_id=self.pad_id,
...
class HFTokenizerAdapter:
    ...
    def _encode_content(self, text: str) -> list[int]:
        ...
        if ids and self.bos_id is not None and ids[0] == int(self.bos_id):
            ids = ids[1:]
        if ids and self.eos_id is not None and ids[-1] == int(self.eos_id):
            ids = ids[:-1]
        return ids
...
def _pack_supervised_ids(
    ...
    *,
    ...
    special_slots = (1 if bos_id is not None else 0) + (1 if eos_id is not None else 0)
    max_content = max(0, max_len - special_slots)
    prompt_ids = list(prompt_ids)
    answer_ids = list(answer_ids)

    if len(prompt_ids) + len(answer_ids) <= max_content:
        prompt_keep = len(prompt_ids)
        answer_keep = len(answer_ids)
    elif len(answer_ids) >= max_content:
        prompt_keep = min(len(prompt_ids), max(0, max_content // 2))
        answer_keep = max(0, max_content - prompt_keep)
    else:
        answer_keep = len(answer_ids)
        prompt_keep = max(0, max_content - answer_keep)

    if answer_ids and answer_keep <= 0 and max_content > 0:
        answer_keep = 1
        prompt_keep = max(0, max_content - answer_keep)

    ids: list[int] = []
    labels: list[int] = []
    if bos_id is not None and len(ids) < max_len:
        ids.append(int(bos_id))
        labels.append(-100)

    kept_prompt = prompt_ids[:prompt_keep]
    ids.extend(int(item) for item in kept_prompt)
    labels.extend([-100] * len(kept_prompt))
    prompt_len = len(ids)

    kept_answer = answer_ids[:answer_keep]
    ids.extend(int(item) for item in kept_answer)
    labels.extend(int(item) for item in kept_answer)

    if eos_id is not None and len(ids) < max_len:
...
class JsonlTextVisionDataset(IterableDataset):
    ...
    def _make_sample(self, row: Dict):
        ...
        if supervised:
            packed = self.tok.encode_supervised(prompt_prefix, answer, self.seq_len)
            ...
            if not text and "controller_signal" in row and prompt and not answer:
                text = prompt
            elif not text:
                text = f"{prompt}\n\n{answer}"
        ...
        if supervised:
            if preference:
                rejected_packed = self.tok.encode_supervised(prompt_prefix, rejected, self.seq_len)
                rejected_ids = rejected_packed["input_ids"]
                rejected_mask = rejected_packed["attention_mask"]
                rejected_labels = rejected_packed["labels"]
